...
* 그룹화된 요소를 컴포지트라고 한다.
* 많은 요소를 가지고있기에 add와 remove 인터페이스가 필요하다.
* **전체 부분을 구성하는 클래스가 동일 인터페이스를 구현하며** 개별, 집합 요소를 같은 타입으로 추상화 한다. 컴포넌트 책임 중요!!
* 컴포지트 기능은 2개로 나뉜다.
	- 컴포넌트 관리
	- 컴포넌트 실행을 컴포지트에 위임 코드 잘 숙지.
* **컴포넌트 패턴 구현 고려 사항 중요!!!**
	- 컴포지트 패턴의 컴포넌트 객체는 메모리에 트리구조를 가진다. ex.) 메뉴속에 > 메뉴, 2진 트리
...
* 헤쉬테그를 가진 생성자와 복사 생성자가 매우중요하다.
* CreateInstance는 인스턴스 필요없이 유닛이 생성되야 함으로 static정적 메소드로 만들어야 한다.
