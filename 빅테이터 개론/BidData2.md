...
## 6. 요인분석의 성분 행렬을 회전시키는 목적
* 공통 용인을 보다 쉽게 해석할 수 있도록 하기 위해서이다.
## 6. 요인분석의 목적
1. 자료를 축소
2. 불필요한 자료를 제거
3. 변수의 구조를 파악
...
## 7. 다중 회기분석의 개념
* 2개 이상의 독립변수와 단일의 종속변수 간의 관계를 설명하는 회귀식
...
## 상관계수
* 상관계수 rxy=1, 혹은 rxy=-1이면 X와 Y는 각각 종속이고, 완전한 상관관계가 있다.
...
* 비율 척도 : 비율 분석이 가능한 척도 
  - 0 개념이 있으며, 사칙연산이 가능함 (ex. 무게, 가격, 시장 점유율, 매출액 등)
...
![Alt text](%EB%8B%A8%EC%9D%BC%ED%91%9C%EB%B3%B8t-.jpg)

### 2. 독립표본 t-검정 : 
![Alt text](%EB%8F%85%EB%A6%BD%ED%91%9C%EB%B3%B8t-.jpg)

### 3. 대응표본 t-검정 : 2개의 대응되는 짝 표본을 추출하여 두표본과 모집단 차이를 구함
![Alt text](%EB%8C%80%EC%9D%91%ED%91%9C%EB%B3%B8t-.jpg)
...
![Alt text](f-%EA%B2%80%EC%A0%95.jpg)

## 카이제곱검정 
![Alt text](%EC%B9%B4%EC%9D%B4%EC%A0%9C%EA%B3%B1%EA%B2%80%EC%A0%95.jpg)
* 2X2 관계 : 각각 표본 값(카이스퀘어)을 더함 ex.) 1번상황 A표본값 + 2번상황 A표본값
* 2Xn 관계 : 카이스퀘어 전부 더하기
...
## 음의 상관관계에서의 값 도출
* A가 존재하여 B에서 도출되는 값은 1보다 작으며 0보다 커야 한다.
...
## 사례기반추론 특징
* 검색 : 사례 데이터베이스를 연동한 검색
* 재사용 : 이전 사례로 부터 대상 문제의 해결방법 연결
* 수정 : 여러 사례를 기반으로 새로운방법을 제시하고 실제상황에 테스트해 필요시 수정을 거침
* 유지 : 다양한 사례들이 모이고 모여 데이터베이스에서 유지됨

# 9장

## 비정형 데이터 
* 비정형 데이터의 정의 : 구체적으로 정의, 정리 되지 않은 정보들
* ex.) 일반 텍스트 문서, 문서를 첨부한 이메일 데이터, 사진과 함께온 문자, 고객 상품 후기
...
## 비정형 데이터가 생성되지 않는 곳
* 상점 계산대 : 상점 물품들은 정형화되어야 소비 되기 때문에

## 텍스트마이닝 
* 대규모 문서에서 정보 추출 등 빅데이터에 숨겨진 의미를 발견하는 기법
* 비정형 텍스트 데이터를 자연어 처리 
* 텍스트 분석, 문서 마이닝으로 불림
* 수집된 언어 자원과 감성 분류, 감성 사전이 매우 중요한 요소로 작용
...
* (웹 콘텐츠 마이닝) 분석에 따른 분류
* (웹 구조) 마이닝에 따은 분류
* (웹에서 패턴)을 발견 하는 것을 말함

## 오피니언 마이닝
* 인물, 이슈, 이벤트, 상황 등의 주관적인 부분을 식별하고 추출하여 분석하는 것을 말한다.
* 의견
* 평가
* 태도
* 감정
...
## 소셜 네트워크 분석
* 노드와 링크로 구성되는 네트워크 이론에 의해서 사회적 관계를 보여주는 것을 말함
* ex.) 우정, 조직력, 성향, 연대감

# 11장

## 빅데이터 분석의 순환 과정 (아래 6개가 순환하다 이용에서 폐기가 될 수 있음)
* 수집
* 저장
* 처리
* 분석
...
## 빅데이터 후처리 저장 단계
1. 빅데이터 후처리 변환 : 데이터를 분석하기 보다 좋은 형식으로 변환
2. 빅데이터 후처리 통합 : 상호 연관성이 있는 데이터를 하나로 통합
3. 빅데이터 후처리 축소 : 데이터의 고유한 특성을 살리면서 효율성이 높게 축소

## 빅데이터를 수집하기 위한 수집 요건
* 확장성 : 데이터 수집 대상이 되는 서버 다수를 무한히 확장 가능
* 안정성 : 수집된 데이터가 소실되지 않고 안정적으로 저장
* 실시간성 : 수집된 데이터를 실시간으로 반영
* 유연성 : 다양한 포멧의 데이터를 지원

## 빅데이터 자동 수집 방법
* 로그 수집기 : 다양한 네트워크 환경, 데이터베이스에서 로그 데이터를 수집
* 크롤링 : 인터넷에 존재하는 자료를 긁어어와 수집
  - 웹 로봇 
  - 웹 크롤러
* 센싱 : 각종 센서를 통한 수집
* 오픈 API : 웹2.0 을 구현하는 기술로 생산, 공유 자원을 프로그래밍으로 수집

데이터 관리 방법 몇가지
빅데이터 관리 절차 아마 시험 안나옴 / 확률기준 같은 것들
빅데이터 관리 특징 이런건 시험 내면 좋을듯
데이터 로그 수집 시험 안냄
